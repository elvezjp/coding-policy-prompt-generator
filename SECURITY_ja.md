...
- CLI はユーザーが明示的に指定した入力・出力・テンプレートのパスを処理します

### 入力検証

- ユーザー指定の列名は照合前に正規化されます
...
本リポジトリは旧バージョンのコードを `versions/` 配下にアーカイブとして保持しているため、それらの lockfile に対しても Dependabot アラートが発生します。以下の方針で運用します。
...
| 旧バージョン（アーカイブ） | **Dismiss**。影響を確認のうえ close |
