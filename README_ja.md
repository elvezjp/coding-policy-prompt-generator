...
- 変更履歴: [English](CHANGELOG.md) / [日本語](CHANGELOG_ja.md)
...
| `--strictness` | `strict` | 判定の厳格度（`strict` or `lenient`） |
| `--project-context` | なし | プロジェクト前提情報（例: "Java 17, Spring Boot 3.x"） |

> 注: カスタムテンプレートでは `規約ID: <rule_id>`（旧形式: `【ルールID】\n<rule_id>`）のマーカーを含めることを推奨します。含まれない場合、再実行時の更新判定ができず新規シートが増える可能性があります。
> 注: Jinjaテンプレートでは `project_context` は未指定時に空文字（`""`）として渡されます（`None` にはなりません）。
...
各ルールにつき1シートを作成し、プロンプト本文を出力します。

- シート名例：`PROMPT_N-001` / `PROMPT_001`
- A1セル：システムプロンプト全文
- A2セル：ユーザープロンプト全文

> **注意**: Excelのセル縦幅には上限があるため、プロンプト全文がセル上で表示されない場合があります。セルの値には全文が格納されているため、セルを選択して数式バーで確認するか、セル（A1:A2）をコピーしてご利用ください。
...
├── src/                    # ソースコード
├── tests/                  # テストコード
├── versions/               # リリースごとのスナップショット（過去バージョンのアーカイブ）
├── CHANGELOG.md            # バージョン履歴（英語）
├── CHANGELOG_ja.md         # バージョン履歴（日本語）
